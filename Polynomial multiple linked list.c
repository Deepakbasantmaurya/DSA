...
{head3=addatend(head3,head2->coeff,head2->pow);
head2=head2->link;
}
}
...
while(ptr1!=NULL)
{while(ptr2!=NULL)
{head3=addatend(head3,ptr1->coeff*ptr2->coeff,ptr1->pow+ptr2->pow);
ptr2=ptr2->link;
}
ptr1=ptr1->link;
ptr2=head2;
...
newp->pow=pow;
newp->link=NULL;
if(temp==NULL||temp->pow<pow)//jab pahle node ka power chotta hai jo naya node banane wala hai to nayanode judega usase pahale 
{newp->link=temp;
temp=newp;
}
else
